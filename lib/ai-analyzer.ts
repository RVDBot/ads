...
Antwoord ALLEEN met een JSON object (GEEN markdown code fences, geen toelichting buiten de JSON). Houd findings kort (max 1-2 zinnen per finding) en beperk tot max 10 findings en max 10 suggesties. Formaat:
{
  ...
  "suggestions": [
    {
      "type": "budget_change|bid_adjustment|keyword_negative|ad_text_change|ad_create|adgroup_create|new_campaign|campaign_targeting|pause_campaign|keyword_add|schedule_change",
...
- **campaign_targeting**: { "campaign_name": "exacte naam", "countries": ["nl","be"], "languages": ["nl","fr"] } — stelt geo-targeting en talen in; gebruik countries[] voor meerdere landen of country voor één land; languages is optioneel (anders standaard talen van het land)
