...
export async function GET(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const metrics = db.prepare(
    'SELECT * FROM daily_metrics WHERE campaign_id = ? AND date >= date(\'now\', \'-\' || ? || \' days\') ORDER BY date DESC'
  ).all(id, days)
  ...
  const searchTerms = db.prepare(`
    ...
    FROM search_terms WHERE campaign_id = ? AND date >= date('now', '-' || ? || ' days')
  ...
  const products = db.prepare(`
    ...
    FROM product_metrics WHERE campaign_id = ? AND date >= date('now', '-' || ? || ' days')
